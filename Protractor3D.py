...
def make_gesture_trace(dataPoints):
        ...
        v = dataPoints
        tp = [[int(v[0][1]),int(v[0][2]),int(v[0][3])]]
        
        for i in xrange(1,len(dataPoints)):
                tp.append(      [tp[i-1][0] + int(v[i][1]),
                                        tp[i-1][1] + int(v[i][2]),
                                        tp[i-1][2] + int(v[i][3])])
        return tp
...
def scale_to_cube(points, CUBE_SIZE = 100):
        ...
        if DEBUG>1: print "scaler", sc
        
        # another classic one-liner (performs the scaling)
        scaled_points = [[v[0] * (s/sc[0]) , v[1] * (s/sc[1]), v[2] * (s/sc[2])] for v in points ]
...
class Protractor3D:
        ...
        def protractor3D_classify(self, gesture, template):
                ...
                N = numpy.array(N_, dtype=numpy.dtype('d') )
